...
func TestMetricUpdatePathHandler(t *testing.T) {
	...
	handler := NewMetricUpdatePathHandler(WithMetricUpdater(mockUpdater))
...
func TestMetricUpdateBodyHandler(t *testing.T) {
	...
	mockUpdater := NewMockMetricUpdater(ctrl)
	handler := NewMetricUpdateBodyHandler(WithMetricUpdater(mockUpdater))
...
func TestMetricUpdatesBodyHandler(t *testing.T) {
	...
	mockUpdater := NewMockMetricUpdater(ctrl)
	handler := NewMetricUpdatesBodyHandler(WithMetricUpdater(mockUpdater))
	...
	tests := []struct {
		...
		{
			name: "Metric missing ID",
			payload: []*types.Metrics{
				{ID: "", MType: types.Counter, Delta: ptrInt64(5)},
			},
			mockExpect:   func() {},
			expectedCode: http.StatusNotFound,
		},
		{
			name: "Counter metric missing Delta",
			payload: []*types.Metrics{
				{ID: "counter1", MType: types.Counter},
			},
			mockExpect:   func() {},
			expectedCode: http.StatusBadRequest,
		},
