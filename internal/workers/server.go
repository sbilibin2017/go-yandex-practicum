...
type ServerWorkerOption func(*ServerWorkerConfig)

func NewServerWorkerConfig(opts ...ServerWorkerOption) *ServerWorkerConfig {
...
// WithRestore sets the restore option.
func WithRestore(restore bool) ServerWorkerOption {
	return func(sw *ServerWorkerConfig) {
		sw.restore = restore
	}
}

// WithStoreInterval sets the interval between store operations.
func WithStoreInterval(interval int) ServerWorkerOption {
	return func(sw *ServerWorkerConfig) {
		sw.storeInterval = interval
	}
}

// WithLister sets the main lister.
...
// WithListerFile sets the file lister.
func WithListerFile(listerFile Lister) ServerWorkerOption {
	return func(sw *ServerWorkerConfig) {
		sw.listerFile = listerFile
	}
}

// WithSaverFile sets the file saver.
func WithSaverFile(saverFile Saver) ServerWorkerOption {
	return func(sw *ServerWorkerConfig) {
		sw.saverFile = saverFile
	}
}

func NewServerWorker(
	opts ...ServerWorkerOption,
) func(ctx context.Context) error {
	config := NewServerWorkerConfig(opts...)
