...
func NewServerApp(opts ...ServerAppOpt) (*ServerApp, error) {
	...
	if cfg.DatabaseDSN != "" {
		...
		app.MetricDBSaveRepository = repositories.NewMetricDBSaveRepository(
			...
			repositories.WithTxGetter(contexts.GetTxFromContext),
		)
	...
	if cfg.DatabaseDSN == "" && cfg.FileStoragePath != "" {
		...
		app.MetricFileSaveRepository = repositories.NewMetricFileSaveRepository(
			repositories.WithFilePath(cfg.FileStoragePath),
		)
	...
	app.MetricUpdatePathHandler = handlers.NewMetricUpdatePathHandler(handlers.WithMetricUpdater(app.MetricUpdatesService))
	app.MetricUpdatePathHandler.RegisterRoute(app.Router)

	app.MetricUpdateBodyHandler = handlers.NewMetricUpdateBodyHandler(handlers.WithMetricUpdater(app.MetricUpdatesService))
	app.MetricUpdateBodyHandler.RegisterRoute(app.Router)

	app.MetricUpdatesBodyHandler = handlers.NewMetricUpdatesBodyHandler(handlers.WithMetricUpdater(app.MetricUpdatesService))
	app.MetricUpdatesBodyHandler.RegisterRoute(app.Router)

	app.MetricGetPathHandler = handlers.NewMetricGetPathHandler(handlers.WithMetricGetter(app.MetricGetService))
	app.MetricGetPathHandler.RegisterRoute(app.Router)

	app.MetricGetBodyHandler = handlers.NewMetricGetBodyHandler(handlers.WithMetricGetter(app.MetricGetService))
