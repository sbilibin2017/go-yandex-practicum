...
func TestDefaultFlags(t *testing.T) {
	setupFlags()
	parseFlags()
	assert.Equal(t, "http://localhost:8080", flagServerURL, "Default server URL should be 'http://localhost:8080'")
...
func TestFlagsFromCommandLine(t *testing.T) {
	setupFlags()
	os.Args = []string{"cmd", "-s", "http://example.com", "-p", "5", "-r", "20", "-l", "debug"}
	parseFlags()
	assert.Equal(t, "http://example.com", flagServerURL, "Flag -s should set the server URL")
...
func TestFlagsFromEnvironmentVariables(t *testing.T) {
	setupFlags()
	os.Setenv("SERVER_URL", "http://env-server.com")
	os.Setenv("POLL_INTERVAL", "3")
	os.Setenv("REPORT_INTERVAL", "15")
	os.Setenv("LOG_LEVEL", "warn")
	parseFlags()
	assert.Equal(t, "http://env-server.com", flagServerURL, "Environment variable SERVER_URL should override the flag")
...
func TestFlagsFromBothCommandLineAndEnvironmentVariables(t *testing.T) {
	setupFlags()
	os.Args = []string{"cmd", "-s", "http://cli.com", "-p", "8", "-r", "25", "-l", "debug"}
	os.Setenv("SERVER_URL", "http://env.com")
	os.Setenv("POLL_INTERVAL", "4")
	os.Setenv("REPORT_INTERVAL", "12")
	os.Setenv("LOG_LEVEL", "error")
	parseFlags()
	assert.Equal(t, "http://env.com", flagServerURL, "Environment variable SERVER_URL should override command-line flag")
